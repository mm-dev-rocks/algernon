...
void main() {
  ...
  for (int arm = 0; arm < 12; arm++) {
    ...
    // Precompute colours for all three depth levels — hoisted out of segment loops
    ...
    if (nDepths >= 2) {
      for (int sub = 0; sub < 2; sub++) {
        ...
        float bAngle = baseAngle + armSway(fi, 1.0) + (bHash - 0.5) * 2.0 + fcArm.y * 0.3;
        ...
        // --- Depth 3: sub-sub-branches from sub-branch tips ---
        if (nDepths >= 3) {
          vec2 bTip = bStart;
