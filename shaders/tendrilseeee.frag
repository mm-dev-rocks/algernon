...
void main() {
  ...
  for (int i = 0; i < 512; i++) {
    ...
    // float energyBoost = myEnergy * 0.5 * (1.0 - abs(windBase) * 2.0);
    float energyBoost = myEnergy * 0.5 * -sign(windBase);
    float swaySignal = (windBase + energyBoost) * u_warp;
    ...
    primarySway =
        (windBase + energyBoost) * u_warp * myWindResp * swayScale * swayFactor;
