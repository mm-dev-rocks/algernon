...
vec2 warpDisplace(vec2 p, float bandLow, float timeOffset, float strength) {
  ...
  float mag    = fc.x;
  float charge = fc.y;
  float freq   = 2.5 + mag * 3.0;
  float angle  = timeOffset + charge * 0.8;

  vec2  pRot = rot2d(p, angle);
  float wx = sin(pRot.x * freq + timeOffset) * cos(pRot.y * freq * 0.7 + timeOffset * 1.3);
  float wy = cos(pRot.x * freq * 0.8 - timeOffset * 0.9) * sin(pRot.y * freq * 1.1 + timeOffset * 0.6);

  return vec2(wx, wy) * strength * (0.5 + mag * 0.8);
}
...
void main() {
  ...
  float bassPhase = u_time * 0.4  + fcBass.x * 5.0 + fcBass.y * 2.0;
  float midPhase  = u_time * 0.65 + fcMid.x  * 3.5 + fcMid.y  * 1.5;
  float highPhase = u_time * 1.1  + fcHigh.x * 2.0 + fcHigh.y * 0.8;

  int  nLayers = int(clamp(u_bandCount, 1.0, 4.0));
  vec2 wp      = p;

  if (nLayers >= 1) wp += warpDisplace(wp, 8.0,   bassPhase,       u_warpStrength * 0.50);
  if (nLayers >= 2) wp += warpDisplace(wp, 48.0,  midPhase,        u_warpStrength * 0.30);
  if (nLayers >= 3) wp += warpDisplace(wp, 120.0, highPhase,       u_warpStrength * 0.15);
  if (nLayers >= 4) wp += warpDisplace(wp, 200.0, highPhase * 1.7, u_warpStrength * 0.07);

  float ink        = inkField(wp * 2.5);
  float brightness = pow(clamp(ink * 0.5 + 0.5, 0.0, 1.0), u_ringContrast);

  float globalCharge = 0.0;
  for (int b = 0; b < 6; b++) globalCharge += sampleBin(float(b) * 10.0).y;
  globalCharge /= 6.0;
  ...
  vec3 finalCol = mix(hsv2rgb(hue, sat, val), hsv2rgb(hue2, sat * 0.8, val * 0.8), fcLocal.x * 0.4);
